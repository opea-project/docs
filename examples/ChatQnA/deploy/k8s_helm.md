...
### Clone Repository 
The next step is to clone GenAIInfra, the containerization and cloud-native suite for OPEA, including artifacts to deploy ChatQnA in a cloud-native way.
...
```
Update the following section and save the file:
```yaml
# chatqna/values.yaml
global:
...
> 
> In a multi-node environment, go to every k8s worker node to ensure that a ${MODELDIR} directory exists and is writable.
> 
...
REVISION: 1
```
It takes a few minutes for all the microservices to get up and running. To verify that the deployment is successful, go to the next section, [Validate Microservices](#validate-microservices).
...
#### Delete link
```
# The dataprep service will add a .txt postfix for the link file
