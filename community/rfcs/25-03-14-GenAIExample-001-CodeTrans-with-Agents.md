...
| Go              | golangci-lint | Runs linters in parallel, reuses Go build cache and caches analysis results                                                                           | [link](https://golangci-lint.run/)                         |
...
case "$LANGUAGE" in
  ...
  go)
    ...
    golangci-lint run "$SOURCE_FILE" > "$REPORT_FILE" 2>&1
    ;;
